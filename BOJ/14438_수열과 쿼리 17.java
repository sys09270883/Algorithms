...
public class Main {
    ...
    private static int update(int idx, int val, int node, int start, int end) {
        ...
        int m = (start + end) / 2;
        return tree[node] = Math.min(update(idx, val, node * 2, start, m), update(idx, val, node * 2 + 1, m + 1, end));
    }
    ...
    private static int getMin(int left, int right, int node, int start, int end) {
        ...
        if (left <= start && end <= right)
            return tree[node];
        int m = (start + end) / 2;
        return Math.min(getMin(left, right, node * 2, start, m), getMin(left, right, node * 2 + 1, m + 1, end));
    }
