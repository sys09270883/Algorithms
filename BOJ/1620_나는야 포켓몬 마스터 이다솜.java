...
오박사 : 다른 아이들은 넝쿨채찍이나, 나뭇잎 공격을 하는데, 다솜이는 역시 뭔가 다르구나. 
그럼 나와 함께 연구소로 가자꾸나. 내가 포켓몬을 한 마리 줄 테니, 너의 꿈을 펼쳐보아라. 꿈은 이루어진단다.
...
그동안의 줄거리 : 이다솜은 일단 상록 숲의 체육관 리더에게 도전을 했다. 
하지만 상록숲 체육관의 리더는 실종된 상태.
 따라서 회색마을부터 도전하기로 했다. 체육관의 리더를 이기면서, 로켓단을 해체시키기도 하고, 여러 가지 사건도 있었다.
  결국 전설의 포켓몬도 잡고, 이제 사천왕을 이기려고 도전하기로 했다. 사천왕은 모두 가볍게 이기고, 이제 마지막 라이벌 오!영!식! 이다.
...
[풀이]
1. LinkedHashMap으로 key에 String, value에 int로 데이터를 넣어서 입력이 숫자인지 문자인지 구분했다.
     하지만 굳이 이렇게 할 필요는 없었던 것 같다.

2. HashMap으로 key에 String, value에 String으로 이름과 순서를 넣고, 바로 찾는게 더 좋다.

걸린 시간 : 1. 508ms > 2. 4804ms
